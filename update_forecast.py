    ...
    try:
        ...
        if os.path.exists(risk_model_path):
            try:
                ...
                # 4. СОХРАНЯЕМ ОБНОВЛЁННУЮ ТАБЛИЦУ РИСКА (после дообучения) ---
                df.to_csv(MASTER_PATH)
        ...
        try:
            fallback = {
                "last_update": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "danger_index": 50.0,
                "danger_status": "⚖️ Стабильно (резерв)",
                "weighted_change": 0,
                "changes": {},
                "forecast": [],
                "weekly": {"days": 7, "interpretation": "Нет данных (резервный режим)."},
                "monthly": {"days": 30, "interpretation": "Нет данных (резервный режим)."}
            }
            with open('data/forecast_result.json', 'w', encoding='utf-8') as f:
                json.dump(fallback, f, indent=2)
            print("⚠️ Записан резервный прогноз, чтобы файл обновился.")
        except Exception as e2:
            print(f"❌ Не удалось записать даже резерв: {e2}")
        return False
